...
class AlhambraTSIMasterMobile:
    def __init__(self):
        ...
        self.cw_wert = 0.32
        # Eindeutiger User-Agent für API-Stabilität - Zeitstempel hilft gegen Cache-Blockaden
        self.geolocator = Nominatim(user_agent=f"alhambra_tsi_mobile_v6192_cloud_{int(time.time())}")
    ...
    def get_coords_cached(_self, adresse):
        ...
        try:
            # Erhöhter Timeout für Cloud-Umgebungen
            time.sleep(1.5)  
            loc = _self.geolocator.geocode(adresse, timeout=15)
            return (loc.latitude, loc.longitude) if loc else None
        except Exception as e:
            st.error(f"Geokodierungs-Fehler bei '{adresse}': {e}")
            return None
    ...
    def get_route(self, punkte):
        ...
        try:
            ...
            if r.get('code') == 'Ok':
                ...
                st.error(f"Routing API Fehler: {r.get('code')}")
                return None
        except Exception as e:
            st.error(f"Verbindungsfehler Routing API: {e}")
            return None
...
# Persistenz der Ergebnisse im Session State
...
with st.sidebar:
    ...
    if st.button("🗺️ Aktuellen Standort (GPS)", use_container_width=True):
        ...
        if loc:
            ...
            st.error("GPS-Signal konnte nicht empfangen werden. Berechtigungen in Chrome prüfen.")
        ...
        with st.status("Alhambra-Engine analysiert Markt & Route...", expanded=True) as status:
            ...
            if s_coords and t_coords:
                ...
                if direkt:
                    ...
                    status.write("📡 Frage Tankerkönig API ab...")
                    ...
                    try:
                        r_tk = requests.get(tk_url, timeout=12)
                        resp = r_tk.json()
                        if resp.get("status") == "error":
                            st.error(f"Tankerkönig API meldet Fehler: {resp.get('message')}")
                            stationen = []
                        else:
                            stationen = resp.get("stations", [])
                            if not stationen:
                                st.warning("Keine Tankstellen im Umkreis von 25km gefunden.")
                    except Exception as e:
                        st.error(f"Verbindungsfehler Tankerkönig: {e}")
                        stationen = []
                    ...
                    status.write(f"🚜 Analysiere {len(stationen)} Stationen...")
                    ...
                    if not st.session_state.results:
                        st.warning("Analyse fertig, aber keine Stationen erfüllten die Umweg-Kriterien.")
                    status.update(label="Analyse abgeschlossen!", state="complete", expanded=False)
                else:
                    st.error("Route konnte nicht berechnet werden (OSRM API Problem).")
            else:
                st.error("Standorte konnten nicht in Koordinaten aufgelöst werden (Geocoder Problem).")
